...
def evaluate(net, dataloader, device, amp, mask_sigma, percentiles=[1, 25, 50, 75, 99], alpha=0.3, downstream=False):
    net.eval()
    ...
    with torch.autocast(device.type if device.type != 'mps' else 'cpu', enabled=amp):
        for images, LV, labels, MD, FA in dataloader:
            ...
            if net.n_classes == 1:
                ...
                LV_dice_score = multiclass_dice_coeff(mask_pred[:, 3], mask_true[:, 3], reduce_batch_first=False)

                weighted_dice_score = alpha * anterior_dice_score + alpha * inferior_dice_score + (1 - 2 * alpha) * LV_dice_score
                ...
                scores.append({"weighted_dice_score": weighted_dice_score, "dice_score" : dice_score, "mse_score" : mse_score, "hausdorff_score" : hausdorff_score, "anterior_dice_score": anterior_dice_score, "inferior_dice_score": inferior_dice_score, "LV_dice_score": LV_dice_score, "mask_true" : mask_true, "mask_pred" : mask_pred, "image" : images})
    ...
    for i, percentile_dice_score in enumerate(percentile_dice_scores):
        ...
        keypoint_file_path, GT_keypoints, pred_keypoints = generate_keypoint_image(mask_true_compiled, mask_pred_compiled, image[0], net.n_classes - 1)
        
        # Generate overlaps images
        image_np = image[0, 0].float().cpu().numpy()
        anterior_overlaps = create_overlap_figure(mask_true[0, 1], mask_pred[0, 1], image_np)
        inferior_overlaps = create_overlap_figure(mask_true[0, 2], mask_pred[0, 2], image_np)
        LV_overlaps = create_overlap_figure(mask_true[0, 3], mask_pred[0, 3], image_np)

        GT_AHA_segmentation = create_AHA_segmentation(GT_keypoints, mask_true[0, 3].float().cpu())
        pred_AHA_segmentation = create_AHA_segmentation(pred_keypoints, mask_pred[0, 3].float().cpu())
        ...
        wandb_logs[f'validation {percentiles[i]}th percentile'] = {
            'masks_panel': [wandb.Image(mask_true_compiled), wandb.Image(mask_pred_compiled), wandb.Image(image[0, 0].float().cpu()), wandb.Image(anterior_overlaps), wandb.Image(inferior_overlaps), wandb.Image(LV_overlaps)],
            ...
            "AHA_segmentations": [wandb.Image(GT_AHA_segmentation), wandb.Image(pred_AHA_segmentation)],
    ...
    for key in ["weighted_dice_score", "dice_score", "mse_score", "hausdorff_score", "anterior_dice_score", "inferior_dice_score", "LV_dice_score"]:
        wandb_logs[key] = sum([score[key] for score in scores]) / N

    net.train()
    return wandb_logs['weighted_dice_score'], wandb_logs
...
if __name__ == "__main__":
    ...
    val_score, wandb_logs = evaluate(net, test_dataloader, device, False, 5, downstream=False)
