...
from evaluate_mask import evaluate
...
def train_model():
    ...
    train_dataset = DataSet(data_path, degrees=rotation, translate=translation, scale=scale, contrast=contrast, flipping=flipping, no_midpoint=no_midpoint, filter_level=filter_level, largest_size=200, use_mask=True)
    val_dataset = DataSet(val_data_path, no_midpoint=no_midpoint, filter_level=filter_level, largest_size=200, use_mask=True)
    ...
    for epoch in range(1, num_epochs + 1):
            ...
            with tqdm(total=n_train, desc=f'Epoch {epoch}/{num_epochs}', unit='img') as pbar:
                for images, heart_mask, _, labels in train_dataloader:
                    ...
                    mask_true = point_masks * non_overlap_mask + (unet.n_classes - 1) * ~non_overlap_mask # combined mask
                        ...
                        if unet.n_classes == 1:
                            ...
                            if 0 < tversky_beta < 1:
                                tversky = tversky_loss(
                                    ...
                                    F.one_hot(mask_true, unet.n_classes).permute(0, 3, 1, 2)[:, 1:].float(),
                                    multiclass=True,
