...
def convert_labels_to_single_mask(labels, height, width, radius=1):
    ...
    assert height == width, "Current implementation multiplies labels by same amount (height). This is only valid for square images."
    ...
    for label in labels:
        ...
        for i in range(len(label) // 2):
            ...
            mask = np.maximum(mask, keypoint_to_radius([one_point], height, width, i+1, radius))
