...
def myocarial_contour_extraction(mask):
    ...
    for slc in range(mask.shape[2]):
        ...
        Mask_Label = skimage.measure.label(
            abs(mask[:, :, slc] - 1), background=None, return_num=False, connectivity=1
        )
        ...
        endocardium_contour = comprehensive_inner_contour(
            Myocardial_Mask[:, :, slc],
...
def Mid_Septum_Calculation(Anterior_RVIP, Inferior_RVIP, Endocardial_Centers):
    ...
    for slc in range(slcs):
        ...
        point_ARVIP = np.array(
            [Anterior_RVIP[slc][0], Anterior_RVIP[slc][1]]
        )  # Static (reference) point
        point_IRVIP = np.array(
            [Inferior_RVIP[slc][0], Inferior_RVIP[slc][1]]
        )  # Static (reference) point
        point_origin = np.array(
            [Endocardial_Centers[slc][0], Endocardial_Centers[slc][1]]
        )  # Origin point
        ...
        radians = np.arctan2(
            np.cross(vector_ARVIP, vector_IRVIP), np.dot(vector_ARVIP, vector_IRVIP)
        )
        ...
        Septum = np.array(
            rotate_vector(point_ARVIP - point_origin, np.radians(mid_angle))
        )
...
def Snap_Septal_Points(Anterior_RVIP, Inferior_RVIP, Mid_Septum, Epicardium_Contours):
    ...
    for slc in range(slcs):
        ...
        Anterior_RVIP_snap[slc] = [
            x_epicardium[min_ARVIP_index],
            y_epicardium[min_ARVIP_index],
        ]
        Inferior_RVIP_snap[slc] = [
            x_epicardium[min_IRVIP_index],
            y_epicardium[min_IRVIP_index],
        ]
        Mid_Septum_snap[slc] = [
            x_epicardium[min_Septum_index],
            y_epicardium[min_Septum_index],
        ]
    return [Anterior_RVIP_snap, Inferior_RVIP_snap, Mid_Septum_snap]


def AHA_17_Segment_Mask(
    mask, Slice_Location, Anterior_RVIP, Mid_Septum, Inferior_RVIP, Endocardial_Centers
):
    import numpy as np
    ...
    for slc in range(slcs):
        if Slice_Location[slc] == "Basal":
            ...
            for x in range(rows):
                for y in range(cols):
                    if mask[x, y, slc] == 1:
                        point_static = np.array(
                            [Mid_Septum[slc][0], Mid_Septum[slc][1]]
                        )  # Static (reference) point
                        point_origin = np.array(
                            [Endocardial_Centers[slc][0], Endocardial_Centers[slc][1]]
                        )  # Origin point
                        ...
                        vector_static = (
                            point_static - point_origin
                        )  # Static (reference) vector
                        vector_moving = point_moving - point_origin
                        radians = np.arctan2(
                            np.cross(vector_static, vector_moving),
                            np.dot(vector_static, vector_moving),
                        )
                        theta = np.degrees(radians)
                        theta = theta
...
def create_AHA_segmentation(keypoints, mask):
    ...
    [Epicardium_Contours, Endocardium_Contours, Endocardial_Centers] = (
        myocarial_contour_extraction(LV_Mask)
    )
    Mid_Septum = Mid_Septum_Calculation(
        Anterior_RVIP, Inferior_RVIP, Endocardial_Centers
    )
    [Anterior_RVIP_snap, Inferior_RVIP_snap, Mid_Septum_snap] = Snap_Septal_Points(
        Anterior_RVIP, Inferior_RVIP, Mid_Septum, Epicardium_Contours
    )

    AHA_Mask = AHA_17_Segment_Mask(
        LV_Mask,
        "Basal",
        Anterior_RVIP_snap,
        Mid_Septum_snap,
    ...
    # Check if AHA_Mask is filled with NaNs
    if np.all(np.isnan(AHA_Mask)):
        return None
