...
def parse_arguments():
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="./data/standard_labels/train",
        help="Path to the data",
    )
    parser.add_argument(
        "--val_data_path",
        type=str,
        default="./data/standard_labels/val",
    ...
    parser.add_argument(
        "--no_midpoint",
        action="store_true",
        help="Toggling whether to include midsegment data",
    )
    parser.add_argument(
        "--test_name_prefix",
        type=str,
        default="",
        help="Something to add onto before the test name",
    )
    parser.add_argument(
        "--filter_level",
        type=int,
        default=0,
        help="0 = no filter, 1 = filter terrible, 2 = filter questionable and terrible",
    )
    ...
    parser.add_argument(
        "--num_features", type=int, default=8, help="Number features in CNN"
    )
    parser.add_argument("--relu", action="store_true", help="Toggle relu")
    parser.add_argument(
        "--dropout", type=float, default=0.5, help="Vary dropout amount"
    )
    parser.add_argument(
        "--early_stopping_patience", type=int, default=20, help="When to early stop"
    )

    parser.add_argument("--lr", type=float, default=1e-6, help="Learning rate for UNet")
    parser.add_argument(
        "--weight_decay", type=float, default=1e-8, help="Weight decay for UNet"
    )
    parser.add_argument(
        "--momentum", type=float, default=0.999, help="Momentum for UNet"
    )
    ...
    parser.add_argument(
        "--CE", type=float, default=0.8, help="Cross Entropy weighting for UNet"
    )
    parser.add_argument(
        "--tversky_beta", type=float, default=-1, help="Tversky beta (F_beta score)"
    )
    parser.add_argument("--amp", action="store_true", help="amp variable for unet")
    parser.add_argument(
        "--save_checkpoint", action="store_true", help="Save UNet checkpoint?"
    )

    parser.add_argument("--rotation", type=int, default=0, help="Rotation amount")
    parser.add_argument(
        "--translation", type=float, default=0.0, help="Translation amount"
    )
