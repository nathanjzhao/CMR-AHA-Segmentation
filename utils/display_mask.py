...
def display_mask(mask, image, name="true"):
    ...
    plt.imshow(
        mask_to_display, cmap="gray"
    )  # Use grayscale color map for better visualization
    ...
    image_to_display = image_to_display.squeeze(
        0
    )  # remove channel axis for greyscale image
    ...
    plt.savefig(
        f"images_{name}.png", bbox_inches="tight", pad_inches=0
